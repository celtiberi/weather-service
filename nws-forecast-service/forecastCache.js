...
function cacheForecast(zoneId, forecast, callback) {
  let issuedAt = new Date();
  let expiresAt = new Date(issuedAt);
  expiresAt.setHours(expiresAt.getHours() + 6); // Default expiry time of 6 hours
  ...
  console.log(
    `Forecast for zone ${zoneId} was issued at ${issuedAt.toUTCString()}`
  );
  console.log(
    `Forecast for zone ${zoneId} will expire at ${expiresAt.toUTCString()}`
  );
  console.log(`ID for the forecast is ${zoneId}`);
  ...
  console.log(
    `Forecast for zone ${zoneId} cached at ${issuedAt.toLocaleString()}`
  );
  console.log(
    `Forecast for zone ${zoneId} expires at ${expiresAt.toLocaleString()}`
  );

  // Schedule the removal of the forecast from the cache
  const timeUntilExpiry = expiresAt - issuedAt;
  setTimeout(() => {
    removeForecastFromCache(zoneId);
...
function getForecastFromCache(zone) {
